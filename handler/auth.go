...
func (h *Handler) SignUp(c *fiber.Ctx) error {
	...
	res, err := h.userStore.Create(input)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(utils.ErrorRes(err.Error()))
	}
...
func (h *Handler) SignIn(c *fiber.Ctx) error {
	...
	signedRes, err := h.userStore.GetOne(strings.ToLower(input.Email), "")
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(utils.ErrorRes(err.Error()))
	}
	...
	accessToken, err := h.userStore.SetAccessToken(signedRes.ID)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(utils.ErrorRes(err.Error()))
	}

	refreshToken, err := h.userStore.SetRefreshToken(signedRes.ID)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(utils.ErrorRes(err.Error()))
	}
...
func (h *Handler) RefreshAccessToken(c *fiber.Ctx) error {
	...
	user, err := h.userStore.GetOne("", userID)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(utils.ErrorRes(err.Error()))
	}

	accessToken, err := h.userStore.SetAccessToken(user.ID)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(utils.ErrorRes(err.Error()))
	}
